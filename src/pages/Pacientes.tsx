      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        ...
        <div className="md:hidden divide-y divide-slate-100">
          {pacientes.map((p) => {
            ...
            const waMsg = encodeURIComponent(`Hola ${p.nombre}, te contactamos desde el consultorio dental.`);
            return (
              <div key={p.id} className="p-4 space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <Link to={`/pacientes/${p.id}`} className="font-semibold text-[#5fb3b0]">
                    {p.nombre} {p.apellido}
                  </Link>
                  <span className="text-xs text-slate-400 shrink-0 pt-0.5">{p._count?.citas ?? 0} citas</span>
                </div>
                <div className="text-sm text-slate-500 space-y-0.5">
                  <p>DNI: <span className="text-slate-700">{p.dni}</span></p>
                  {p.obraSocial?.nombre && <p>OS: <span className="text-slate-700">{p.obraSocial.nombre}</span></p>}
                  {(p.telefono || p.email) && <p>{p.telefono || p.email}</p>}
                </div>
                <div className="flex items-center gap-3 pt-1 flex-wrap">
                  <Link to={`/pacientes/${p.id}`} className="text-sm text-[#5fb3b0] hover:underline">Ver ficha</Link>
                  <button type="button" onClick={() => abrirEdicion(p)} className="text-sm text-slate-500 hover:underline">Editar</button>
                  {tel && (
          ...
          {pacientes.length === 0 && !showForm && (
            <p className="text-center py-12 text-slate-500">No hay pacientes. Crea uno para comenzar.</p>
          )}
        </div>
                ...
                return (
                  <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50">
                    <td className="px-4 py-3">
                      <Link to={`/pacientes/${p.id}`} className="font-medium text-[#5fb3b0] hover:underline">
                        {p.nombre} {p.apellido}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-slate-600">{p.dni}</td>
                    <td className="px-4 py-3 text-slate-600">{p.obraSocial?.nombre || '-'}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <Link to={`/pacientes/${p.id}`} className="text-sm text-[#5fb3b0] hover:underline">Ver ficha</Link>
                        <button type="button" onClick={() => abrirEdicion(p)} className="text-sm text-slate-500 hover:text-slate-700 hover:underline">Editar</button>
                        {tel && (
                          <a href={`https://wa.me/${tel}?text=${waMsg}`} target="_blank" rel="noopener noreferrer"
